...
import { ReceiptData } from '@/types/accounts';
import { useBanks } from '@/hooks/useBanks';

interface ReceiptModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (receiptData: ReceiptData) => Promise<void>;
  accountDescription: string;
  loading?: boolean;
}

export function ReceiptModal({ isOpen, onClose, onConfirm, accountDescription, loading = false }: ReceiptModalProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.received_at) {
      newErrors.received_at = 'Data de recebimento é obrigatória';
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md bg-white/95 backdrop-blur-xl border border-white/20">
        ...
        <div className="mb-4 p-3 bg-green-50/80 rounded-lg border border-green-200/50">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className={errors.received_at ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : ''}
            />
            {errors.received_at && <p className="text-sm text-red-600">{errors.received_at}</p>}
          </div>
