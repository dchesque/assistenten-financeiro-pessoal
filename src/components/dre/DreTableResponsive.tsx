...
interface DreTableResponsiveProps {
  dados: DreItem[];
  periodo: string;
...
export function DreTableResponsive({ 
  dados, 
  periodo, 
  periodoComparacao, 
  mostrarComparacao, 
  tipoVisualizacao 
}: DreTableResponsiveProps) {
  ...
  const temFilhos = (item: DreItem, index: number) => {
    const proximoItem = dados[index + 1];
    return proximoItem && proximoItem.nivel > item.nivel;
  };

  const deveExibir = (item: DreItem, index: number) => {
    if (item.nivel === 0) return true;
    
    // Encontrar o item pai
    for (let i = index - 1; i >= 0; i--) {
      const itemAnterior = dados[i];
      if (itemAnterior.nivel < item.nivel) {
        return expandidos.has(itemAnterior.codigo);
      }
    }
    return true;
  };
      ...
      <CardContent className="p-0">
          ...
          <div className="min-w-full">
            ...
            <div className="divide-y divide-gray-100">
              {dados.map((item, index) => {
                if (!deveExibir(item, index)) return null;
                ...
                return (
                  <div 
                    key={item.codigo}
                    className={`grid grid-cols-12 gap-2 p-4 transition-all duration-200 hover:bg-white/40 ${
                      item.tipo === 'total' ? 'font-bold text-lg border-t-2 border-b border-gray-300 bg-gradient-to-r from-blue-50/60 to-purple-50/60' :
                      item.tipo === 'subtotal' ? 'font-semibold text-base border-t border-gray-200 bg-gradient-to-r from-gray-50/60 to-blue-50/40' :
                      item.nivel > 1 ? 'text-sm text-muted-foreground' : ''
                    }`}
                    style={{ marginLeft: `${item.nivel * 12}px` }}
                  >
                    {/* Código */}
        ...
        <div className="lg:hidden space-y-2 p-4">
          {dados.map((item, index) => {
            ...
            const variacao = item.valorComparacao ? 
              calcularVariacao(item.valor, item.valorComparacao) : 0;
            const possuiFilhos = temFilhos(item, index);
            const estaExpandido = expandidos.has(item.codigo);

            return (
              <div
                key={item.codigo}
                className={`bg-white/60 backdrop-blur-sm rounded-xl border border-gray-200/50 shadow-sm ${
                  item.tipo === 'total' || item.tipo === 'subtotal' ? 'border-l-4 border-l-blue-500' : ''
                }`}
                style={{ marginLeft: `${item.nivel * 12}px` }}
              >
                <div className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className={`text-right font-medium ${obterCorPorTipo(item.tipo, item.valor)}`}>
                      {formatarMoeda(item.valor)}
                    </div>
                  </div>
                  ...
                  <div className="mb-2">
                    <span className={`${obterCorPorTipo(item.tipo, item.valor)} ${
                      item.tipo === 'total' || item.tipo === 'subtotal' ? 'font-semibold text-base' : ''
                    }`}>
                      {item.nome}
                    </span>
                  </div>

                  {mostrarComparacao && item.valorComparacao && (
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">
                        Anterior: {formatarMoeda(item.valorComparacao)}
                      </span>
                      <div className="flex items-center gap-1">
                        {obterIconeVariacao(variacao)}
                        <span className={`text-xs font-medium ${
