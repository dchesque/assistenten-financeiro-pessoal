// Componentes base reutilizáveis para modais de vendas
...
// === GRID DE VALORES FINANCEIROS PARA VENDAS ===
interface ValorVendaGridProps {
  valorBruto: number;
  valorDesconto?: number;
  valorLiquido: number;
  percentualDesconto?: number;
  tipoVenda: 'venda' | 'devolucao' | 'desconto';
}

export const ValorVendaGrid = ({ valorBruto, valorDesconto, valorLiquido, percentualDesconto, tipoVenda }: ValorVendaGridProps) => (
  <div className="space-y-6">
    <FieldDisplay label="Valor Bruto" value={new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(valorBruto)} />
    
    {valorDesconto && valorDesconto > 0 && (
      <FieldDisplay 
        label="Desconto" 
        value={`${percentualDesconto ? `${percentualDesconto.toFixed(1)}% - ` : ''}${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(valorDesconto)}`}
    ...
    <div className={`border rounded-2xl p-6 ${
      ...
      <div className="flex justify-between items-center">
        ...
        <span className={`text-3xl font-bold ${
          tipoVenda === 'devolucao' ? 'text-red-600' : 'text-blue-600'
        }`}>
          {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Math.abs(valorLiquido))}
        </span>
      </div>
      {tipoVenda === 'devolucao' && (
