...
-- Para queries de relatórios complexos (contas por período e categoria)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_accounts_payable_reports
ON accounts_payable(user_id, category_id, date_part('month', due_date), date_part('year', due_date))
WHERE deleted_at IS NULL;

-- Para queries de fluxo de caixa (valor e data)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transactions_cashflow
ON transactions(user_id, date, type, amount)
WHERE deleted_at IS NULL;

-- Para contatos por tipo (fornecedor, cliente, etc)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_type_active
