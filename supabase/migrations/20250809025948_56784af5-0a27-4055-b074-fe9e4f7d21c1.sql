-- FASE 1: Configuração Base + Autenticação Email + Schema Fixes
...
CREATE INDEX CONCURRENTLY idx_accounts_payable_active ON accounts_payable (user_id, due_date) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY idx_accounts_receivable_active ON accounts_receivable (user_id, due_date) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY idx_suppliers_active ON suppliers (user_id, name) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY idx_categories_active ON categories (user_id, name) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY idx_banks_active ON banks (user_id, name) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY idx_bank_accounts_active ON bank_accounts (bank_id) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY idx_transactions_active ON transactions (user_id, date) WHERE deleted_at IS NULL;
...
-- ============================================================================
-- TAREFA 7: Função para migração gradual suppliers -> contacts
-- ============================================================================

-- Função para migrar um fornecedor específico para contato
CREATE OR REPLACE FUNCTION migrate_supplier_to_contact(p_supplier_id UUID)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  -- Buscar fornecedor
  SELECT * INTO supplier_record FROM suppliers 
  WHERE id = p_supplier_id AND deleted_at IS NULL;
  
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Fornecedor não encontrado ou já deletado';
  END IF;
  
  -- Criar contato correspondente
  INSERT INTO contacts (
    user_id, name, type, document, email, phone, 
    address, city, state, zip, active, notes
  ) VALUES (
    supplier_record.user_id,
    supplier_record.name,
    'supplier',
    supplier_record.document,
    supplier_record.email,
    supplier_record.phone,
    supplier_record.address,
    supplier_record.city,
    supplier_record.state,
    supplier_record.zip,
    supplier_record.active,
    supplier_record.notes
  ) RETURNING id INTO new_contact_id;
  
  -- Atualizar contas a pagar que usavam este fornecedor
  UPDATE accounts_payable 
  SET contact_id = new_contact_id 
  WHERE supplier_id = p_supplier_id AND deleted_at IS NULL;
  
  -- Soft delete do fornecedor original
  UPDATE suppliers 
  ...
  VALUES (
    ...
    p_supplier_id::TEXT,
    jsonb_build_object(
      'migrated_to_contact_id', new_contact_id,
      'accounts_updated', (SELECT COUNT(*) FROM accounts_payable WHERE contact_id = new_contact_id)
    )
  );
