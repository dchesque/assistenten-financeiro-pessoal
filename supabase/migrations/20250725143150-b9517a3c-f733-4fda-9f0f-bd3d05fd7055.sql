...
-- Trigger para atualizar estatísticas do cliente quando venda for criada/atualizada
CREATE TRIGGER trigger_atualizar_estatisticas_cliente_vendas
AFTER INSERT OR UPDATE OR DELETE ON public.vendas
FOR EACH ROW
EXECUTE FUNCTION public.atualizar_estatisticas_cliente_completas(
  CASE 
    WHEN TG_OP = 'DELETE' THEN OLD.cliente_id 
    ELSE NEW.cliente_id 
  END
);
