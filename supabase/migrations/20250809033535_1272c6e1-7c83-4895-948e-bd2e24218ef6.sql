-- FASE 1: Schema Fixes + Soft Delete + Contacts System
-- Migration: 20250109_phase1_schema_fixes.sql
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_accounts_payable_active ON accounts_payable(user_id, due_date) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_accounts_receivable_active ON accounts_receivable(user_id, due_date) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_banks_active ON banks(user_id) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_categories_active ON categories(user_id) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_suppliers_active ON suppliers(user_id) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transactions_active ON transactions(user_id, date) WHERE deleted_at IS NULL;
...
ALTER TABLE accounts_payable ADD COLUMN IF NOT EXISTS contact_id UUID REFERENCES contacts(id);
ALTER TABLE accounts_receivable ADD COLUMN IF NOT EXISTS contact_id UUID REFERENCES contacts(id);
...
-- 7. Criar funções auxiliares para soft delete
...
-- 8. Criar trigger para updated_at em contacts
CREATE TRIGGER update_contacts_updated_at
    BEFORE UPDATE ON contacts
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- 9. Criar índices para contacts
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_user_active ON contacts(user_id) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_type ON contacts(type) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_document ON contacts(document) WHERE deleted_at IS NULL AND document IS NOT NULL;

-- 10. Criar constraint único para documento por usuário
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_user_document_unique 
ON contacts(user_id, document) 
