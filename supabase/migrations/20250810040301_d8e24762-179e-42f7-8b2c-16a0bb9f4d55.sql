-- Fase 1: Limpeza definitiva do banco de dados
-- Remover todas as referências obsoletas a suppliers, fornecedores e plano_contas

-- 1. Dropar views que dependem da tabela suppliers
...
-- 3. Remover a coluna supplier_id da tabela accounts_payable 
-- (já temos contact_id que é o correto)
ALTER TABLE public.accounts_payable 
DROP COLUMN IF EXISTS supplier_id;

-- 4. Verificar se contact_id tem foreign key constraint correta
-- Primeiro dropar se existir constraint antiga
DO $$ 
BEGIN
    IF EXISTS (
...
-- 9. Atualizar comentários das tabelas para refletir a nova estrutura
