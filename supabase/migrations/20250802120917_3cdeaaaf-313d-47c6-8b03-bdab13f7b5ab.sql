...
DROP FUNCTION IF EXISTS public.estatisticas_contas_rapidas(uuid);

-- Recriar função para buscar contas otimizada que aceita filtros string/numeric
...
BEGIN
    ...
    SELECT 
        ...
        cp.status,
        cp.data_pagamento,
        COALESCE(cp.valor_pago, cp.valor_final) as valor_pago,
        cp.grupo_lancamento,
...
-- Recriar função para estatísticas rápidas
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(SUM(valor_pago) FILTER (WHERE status = 'pago' AND data_pagamento >= date_trunc('month', CURRENT_DATE)), 0) as valor_pago_mes
    FROM contas_pagar
